...
- **空白返回**：点地图空白处退回上一层范围（次区域→大洲→全世界；省→全国），分段按钮高亮同步回退。答题进行中不生效（提示「请重置后返回全国」）。
...
- 世界记忆单位：195 个答题国家（联合国会员国及观察员国）；属地/南极等为装饰面，不参与答题。答题国按 6 大洲、23 个次区域划分。
...
scripts/build-subregions.mjs    # 次区域数据：195 国 → 23 个次区域映射（含 6 组一致性断言）
