...
- `src/puzzle/view.ts` + `src/modes/puzzle.ts` —— SVG 渲染、pointer 拖拽（含"点选→点放"）、以光标为锚点的滚轮缩放（0.6–6x）、拖空白平移、双击空白复位、`ResizeObserver` 跟随容器尺寸；**面积排序（小的压大的）靠每片一个 `<g class="puzzle-piece-wrap">` 按面积降序排 DOM**（一组一个 `<g>` 做不到全局排序）；模式负责范围/计时/难度/提示/获胜流程。
...
**默认视角**：当前范围碎片 bbox 宽 ≈ 视口宽 ×1.15（省级全国约 27px/°）。**港澳在默认比例下只有约 1.6px 宽** —— 从卡槽拖出不受影响（拖的是槽里的放大预览），放下后要单独再挪它需先放大；这正是允许 0.6–6x 缩放的原因。

**获胜流程**：最后一组吸合 → 补三沙岛礁（淡入，归入海南组；只有省级档有）→ 镜头动画到"刚好装下整图"并居中（**连带岛礁一起装进来**，否则那句"自动补上"看不见）→ 弹完成卡片（用时 + 再来一局/关闭）。**不计步数、不进排行榜**（用户口径）。
