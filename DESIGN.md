...
- `src/puzzle/pieces.ts` —— 三个来源：世界档取 `worldGeoJson` 的 194 个答题国（按 `continent` / `isoSubregion` 收窄）；省级档取省级 plus（40%）档的 34 片；市级档取地级 plus 档 ∩ `data.units`（按 `provinceAdcode` 过滤）。**南海诸岛装饰面**（`100000_JD`）与全部 `decorative: true` 面不作碎片；**海南**按"主岛 + 主岛 bbox 外扩 0.5° 内的近岸小岛"当碎片，其余 200 多个远海岛礁进 `seaIslets`，获胜后自动补上。
  **拼图固定吃 40%（plus）档，不随缩放换档**（用户 2026-09-14 确认）：地图页是"一屏只画视口内的面"，可以靠换档 + 裁剪压住开销；拼图要把**该范围全部碎片同时留在画布上**（最多 340 片），越档重建会一次性重算 340 条 `path d`，换来的是放大到 20x 以上才看得出的折线差别 —— 不值得。五档阶梯与实测顶点/体积见 §4.2（4% 11,464 / 8% 18,173 / 15% 29,828 / 40% 71,509 / 100% 173,337）。
...
- `src/puzzle/state.ts` —— 位置模型：`<path>` 的 `d` 永远是**真值投影**，用户拖动只改所属组的偏移 `(dx,dy)`；于是"两片摆对了"= 两组偏移相等。`drop()` 是**手里那片主动吸过去**（用户口径 2026-09-14 改过方向：早先是"目标组移动到手里这块的位置"，现在反过来 —— 拖拽方跳到目标的偏移上，连锁时依次跳到各目标的位置），可连锁合并；`isComplete()` = 全部碎片同属一组；`assembledCount()` = `min(总片数, 1 + 累计吸收的组数)`（用户口径：起始 1，每次吸附 +1）；`groupArea()` = 组内各片面积之和（画布叠放用）。
- **磁吸容差按难度分档**（用户口径 2026-09-14）：`SNAP_TOLERANCE_PX = { easy: 10, hard: 5 }`（原来是两档统一 15px）。难度在运行中锁定，所以 `ensureState()` 建一次就够；提示范围与容差是同一个值（提示只预告真会吸上的情形）。
...
**难度只管名称、容差与预告**（用户口径）：`PuzzleView` 的 `labels()` 控制名称、`hints()` 控制可吸附提示；困难档 `hints()` 直接 `clearHighlights()` 返回——拖动中目标组不亮、手里那块不加描边、点选→点放的幽灵预览也不亮，只有松手后真正吸上才看得见结果；简单档的提示**只有边界**（2.5px 绿描边 + `drop-shadow` 绿辉光；**不铺整片面积的绿色填充** —— 用户口径 2026-09-14，填充会把碎片整块染绿、盖掉底下的边界）。容差 10/5px 见上一条。

**画布叠放：先组、后片**（用户口径 2026-09-14 的第二版）：`renderStructure()` 按 **`(组总面积 desc, 片自身面积 desc, groupId, adcode)`** 排序 DOM —— SVG 的上下覆盖就是 DOM 顺序。于是：
- **许多小碎片拼成的大块会沉到中层碎片之下**（组面积是**组内各片之和**，不是"这片很小所以浮在上面"）；
- 组内仍旧是**小的压在大的之上**（北京/天津陷在河北的环里、港澳压在广东之上）；
- 没拼的单片自成一组、组面积 = 自身面积，行为与第一版一致。
两级面积都写进 DOM（`data-area` / `data-group-area`）供验收断言。
...
- **可吸附提示太弱**：原提示只有目标组一条 2px 绿边，困难模式（无名称、同色灰面）几乎看不见；后来简单档改为 2.5px 绿边 + 绿辉光 + 半透明绿填充，并通过 `PuzzleState.candidatesFor()` 同时覆盖拖动与"点选→点放"；本轮困难档按新口径**完全没有提示**、简单档**只留边界荧光**（去掉整片填充）。
- **磁吸方向与容差**：第一版是"目标组移动到手里这块的位置"（避免松手瞬间跳走）+ 两档统一 15px；本轮按用户口径改为**拖拽方主动吸附过去**，并把容差分档为简单 10px / 困难 5px。
- **面积层级**：第一版是全局按**单片**面积排序；本轮改为先按**组总面积**、组内再按单片面积（见上）。
- **港澳在旧口径下约 1.6px 宽**（当时 1x ≈ 26px/°）→ 改成"1x = 地图 1x"后 1x 下更小（约 0.54px），但缩放上限提到 28x 后可放到约 15px —— 这才是把缩放范围与地图对齐的实际收益。

---

## 18. 下钻后隐藏无关地区（2026-09 新增的全局开关）

**需求（用户口径）**：全局设置里加一个开关「**下钻后隐藏无关地区**」，**默认打开**；打开 = 现在的样子（下钻到次级区域后无关地区看不见）；关闭 = 下钻后**其他地区依然显示**，但**无法交互**，且填充为**比空白底色更深一档的浅灰**；关闭时下钻到省份，**依然保持五级精细度分段**。

### 18.1 一条开关改三处渲染结果

| 作用点 | 开（默认） | 关 | 代码位置 |
|---|---|---|---|
| 范围外的面**可见性** | 透明（看不见） | 浅灰 `#b0b5bd`（暗主题 `#2b3441`） | `map/layers.ts` 的 `outOfScopeFill()`（地级与世界两个分支共用） |
| 范围外的面**可交互性** | `silent`（不高亮/不给 tooltip/点击等同点空白） | **同样是 `silent`**（不变） | 同上（`silent: !inView` / `silent: true`） |
| 下钻时的**省界线** | 只画当前省 | 邻省省界照画（否则整片浅灰糊成一块） | `renderer.buildLineData()` |
| 下钻时的**精细度档** | 强制 lossless | 按 zoom 走五档 | `map/tiers.ts` 的 `drillForcesLossless()` |

**为什么"可交互性"不能跟着变**：范围外的面本来就登记为 `silent`（否则 ECharts 会让它回落到 geo 默认样式——看不见却能悬停高亮、点了还会跳到上级区域，见 §ADR 0005 / CONTEXT「惰性面」）。本开关只切换**外观**，`silent` 恒定，于是关闭后得到的是「**看得见的惰性面**」：占着位置、画着浅灰，但依旧点不动。断言在 `layers.test.ts`（单测）与 `verify-drill-scope.mjs`（真实指针：悬停灰区画布**零变化**，而悬停本省面会重绘——后者是对照组，证明这条断言方法本身有效）。

**为什么关闭后必须放弃"强制无损档"**：开着时下钻后视口里只剩当前省，顶点再多也被 `cull.ts` 的视口裁剪挡住（`ignore` 让 zrender 直接跳过 `buildPath`），所以直接给最精细档最划算。关掉后邻省/邻洲**仍在绘制**，若还强制 lossless，那些面会顶着 100% 顶点的几何参与每帧构建——拖动缩放明显掉帧。故此时回到按 zoom 分档，这也正是用户要求的「保持五级精细度分段」。

### 18.2 颜色与观感

- **浅灰必须比空白更深**：地图空白处是 `#map { background: var(--map-bg) }`（明 `#d1d5db` / 暗 `#374151`），新增主题令牌 `MapTheme.inactiveFill`（明 `#b0b5bd` / 暗 `#2b3441`）**深一档**，亮度差 ≈ 32（明主题），肉眼一眼能分清「这片是范围外的地区」与「这里本来就没有内容」。
- **世界档照画国界**（`worldBoundaryTone`，宽 0.4）：世界图的国家边界本来就画在 `geo.regions` 上，若范围外的面不给边界，整片浅灰会糊成一块看不出国别；中国档同理保留**省界折线**（地级市边界不画，与下钻视图对范围外的一贯做法一致）。
- **地级面优先于金币着色**：无尽闯关的金币色只在范围内生效，范围外的面一律浅灰（曾经的三元表达式顺序会金币色覆盖范围外判定，已调整并加单测）。
- **标签不动**：范围外的面不产生地名标签（本来就被 `viewProvince` 过滤），灰区上没有文字。

### 18.3 设置项与生效路径

- `Settings.hideUnrelatedOnDrill`（**默认 true**，普通布尔；`loadSettings` 里对非法值显式归一，旧档缺该字段 → 默认 true = 观感不变）；
- 面板：`index.html` 新增「地图下钻」分区 + `#set-hide-unrelated-drill`（沿用 `.row.setting-row` 排版：文字左、开关右）；
- 生效：`MapRenderer.setHideUnrelatedOnDrill()` —— 启动时灌一次、保存设置时再灌一次；它会整块 `render()`（因为档位可能换地图名，`render()` 自带的地图名检测会走 `replaceMerge` 换图），**不必重进下钻视图**，当前画面当场变。
- 只影响渲染与档位：练习范围、出题池、熟练度、排行榜一概不变（与「忽略面积极小的国家」同一性质的观感开关）。

### 18.4 验收

- 单测（`npm test`）：`layers.test.ts`（两种取值下的面样式/静默/边界/浅灰比底色深/金币优先级）、`tiers.test.ts`（`drillForcesLossless` + 关闭后五档仍然齐全）、`store.test.ts`（默认 true、旧档回落、脏值归一、往返）。
- 运行时（`npm run build && node scripts/verify-drill-scope.mjs`，34 项，截图 `docs/shots/drill-scope-*.png`）：设置面板默认勾选与排版、开/关两种取值下的 `geo.regions` 回读、省界线画了哪些省、档位是否跟着 zoom 走、**真实鼠标**悬停/点击灰区、**同屏像素直方图**（关闭后多出的浅灰格数 ≈ 少掉的空白格数：621 ≈ 655−34，差值是画在灰面上的省界与标签）、暗主题、世界档、以及"重新打开即恢复默认观感"的回归闸门。
