...
**玩法**（用户四轮 grill 定稿）：进入时**没有地图**；左侧三个玻璃态**卡槽**各装一片省级单位；拖出即 1:1 真值大小、可放任意位置；**松手时**相邻两片若接近真值相对位置（≤15 拼图像素）就**精确对齐**并为**拼图组**，此后整组一起拖；34 片全部落下且吸成**一整块**即获胜。
...
- `src/puzzle/view.ts` + `src/modes/puzzle.ts` —— SVG 渲染、pointer 拖拽（含"点选→点放"）、以光标为锚点的滚轮缩放（0.6–6x）、拖空白平移、双击空白复位、`ResizeObserver` 跟随容器尺寸；模式负责计时、难度、提示、获胜流程。

**默认视角**：整幅中国 bbox 宽 ≈ 视口宽 ×1.15（约 27px/°）。**港澳在默认比例下只有约 1.6px 宽** —— 从卡槽拖出不受影响（拖的是槽里的放大预览），放下后要单独再挪它需先放大；这正是允许 0.6–6x 缩放的原因。

**获胜流程**：最后一组吸合 → 补三沙岛礁（淡入，归入海南组）→ 镜头动画到"刚好装下整图"并居中（**连带岛礁一起装进来**，否则那句"自动补上"看不见）→ 弹完成卡片（用时 + 再来一局/关闭）。**不计步数、不进排行榜**（用户口径）。

**暂停**：与其它模式同一套"点击继续"遮罩；暂停与切走都不累计时间（`pause()` 必须先 `commitElapsed()` 再置暂停标志，顺序反了会丢掉整段——实测踩过一次）。

**本轮范围**：只做**全国省级**（34 片）。粒度分段按钮三档都显示，点世界/市级提示"暂未开放"并停在省级；难度分段按钮**只管是否显示省名**（简单=显示、困难=不显示，持久化）。