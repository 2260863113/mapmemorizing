...
- 运行期实际同步加载 14 个文件（`Promise.all` 并行）：地级五档（124 / 158 / 215 / 416 / 908KB）+ 省级五档（43 / 58 / 84 / 174 / 396KB）+ `hkmac.geojson` 125KB + 元数据 `units.json` 84KB / `countries.json` 30KB / `world_v2.topojson` 636KB。合计 **3452KB**。
  ...
  - **世界图用 Robinson 投影**（见 4.5），故其 `boundingCoords` 取经纬度框 `[[-180,-90],[180,83.6]]` 后由 `geo.projection` 映射，不再是等距圆柱。
...
**实现**（`src/map/cull.ts`）：
- 视口换算：用 `geoCoordSys.pointToData()` 取画布四角 → 数据坐标 bbox。投影经 `boundingCoords` 钉死后 lng/lat→像素为线性映射，四角即覆盖整个视口。（此路径只在中国图上生效——`cullToViewport()` 对世界模式直接 early-return；且 `pointToData` 即便在世界图的 Robinson 投影下仍收/发经纬度，因为 ECharts 内部自行施加 project/unproject。）
...
7. **数据简化策略（四档 + 无损档 + 视口裁剪，拓扑保持 + explode + 单一来源）**：数据源 2026-09 从阿里 DataV 换成 cn-atlas（TopoJSON 共享弧，相邻边 0% 零共享，根治地图缝隙）；全部 372 个 `prefectures` 面（地级 + 县级/兵团市，本在同一套弧拓扑里）整体取用，**不做 `-snap`/`-clean` 缝合**（该步骤会在面上开洞造成露白，详见 4.2），只额外补入 cn-atlas 没有的南海诸岛面。地级地图按 zoom 分五档（无损 ≥14 / plus 10~14 / fine 6~10 / pro 2~6 / ultra <2），省级同用一套阈值，简化档由 mapshaper `visvalingam keep-shapes` 拓扑简化（共享弧只简化一次，不产生缝隙），最精细档**完全不简化**，其流畅度由**运行时视口裁剪**保证（见 4.4）——
   ...
   - **固定投影范围（boundingCoords）**：ECharts 默认按**当前注册地图的几何 bbox** 自动适配投影，而各简化档的 bbox 并不严格相同（ultra/省级粗档把南海诸岛最南端简化掉，纬度下界 3.3974 → 3.5349，高度少 0.1375°≈15km）。bbox 一变投影比例与居中偏移就变 → 缩放跨换档阈值时整幅地图微移、鼠标所指位置偏移。实测换档偏移 **13.8px**（这就是"跨档缩放时地图小幅度移动"的根因）。修法：`geo.boundingCoords` 钉死投影范围为常量（中国族用 `CHINA_PROJECTION_BBOX`、世界族用 `WORLD_BOUNDING_COORDS`），中国族各档与地级/省级两族共用同一投影。修复后偏移降至 **0.003px**（zrender 取整噪声量级）。注意 bbox 完全相同的两档（如 fine↔pro）本来就无偏移，问题只出在把南海诸岛最南端简化掉的粗档上。
      - **世界图的 boundingCoords 动机不同**：世界族只有一个数据档，故不是为了「换档不位移」，而是取景基准确定、可复核（将来几何再变——例如回补南海岛礁——投影比例与居中位置都不跟着变）。取值 `WORLD_BOUNDING_COORDS = [[-180,-90],[180,83.6]]`（`src/map/projection.ts`）。
    - **世界图投影为 Robinson（`src/map/projection.ts`）**：等距圆柱不保面积，实测俄罗斯在图上面积是真实的 **2.15 倍**、格陵兰 **3.82 倍**、加拿大 2.11 倍，以致「俄罗斯看起来比非洲还大」（真实比值只有 **0.574**，等距圆柱下却是 **1.164**）；Robinson 把它们收敛到 1.50 / 1.96 / 1.48 倍，俄非比值回到 **0.828**。中国图**不改**（纬度跨度仅 3.4–53.6°N，收益小，而它与五档换档/视口裁剪/港澳放大框/下钻联动很密）。Robinson 用官方 19 行系数表（0–90° 步长 5°）线性插值，`x = 0.8487·X(φ)·λ`、`y = 1.3523·Y(φ)`；数学由 `src/map/projection.test.ts` 的 23 条断言钉死（官方系数表端点、往返一致性、面积失真必须小于等距圆柱）。
    - **投影改变 `geo.center` 的语义（最大的接入陷阱）**：一旦提供 `geo.projection`，ECharts 的 `center` **从经纬度变为投影后坐标**（`node_modules/echarts/lib/action/roamHelper.js` 原注释：「Use projected coord as center because it's linear」），`getBoundingRect()` 亦然；而 `dataToPoint`/`pointToData` 仍收/发经纬度。实测把旧值 `center: [0,-3.2]`（经纬度）直接喂给启用投影的 geo，北京落到 y=765.6（460px 画布外，地图整个错位）；改用投影后的纵向中点 `[0,-0.0231]` 则落在 (670,326)，正确。
      - 对策：渲染器内部**以经纬度为相机的规范表示**（`this.center` 恒为 `[lng, lat]`），投影只在 `toGeoCenter()` / `fromGeoCenter()` 这一对转换里出现。6 处 `setOption({geo:{center}})` 全部改走 `toGeoCenter`，2 处回读（`georoam` 从 geo 读权威中心、`currentGeoView` 从 `chart.getOption()` 读）改走 `fromGeoCenter`。
      - 这样安排的额外好处：`savedWorldView`/`savedChinaView`/`savedNationView` 三个快照槽存的是经纬度，各自属于不同的地图族也不会串味；`countries.json` 的 `center` 与 `modes/input.ts` 里 `closestUnvisited` 的 center 对 center 比较**都是经纬度对经纬度，与投影无关**，无需改动。
      - 世界图默认中心的纬度改为**由投影反算**：Robinson 下「南极到 83.6°N」关于赤道不对称（南半球多 6.4°），投影包围盒纵向中点在约 **-1.378°**，不是赤道；取旧值 -3.2 会让世界图上下偏约 1.2% 画布高。该值由 `robinsonUnproject` 现算，随公式联动，不手抄常数。
      - 大洲取景 zoom 公式随之改写，且必须**宽高双适配**：原式 `360/经度跨度` 只在等距圆柱下成立；Robinson 下同一段经度的投影宽度随纬度收缩（X(φ)：赤道 1.0 → 60°N 0.80 → 80°N 0.62）。但只按宽度反推会把画面纵向撑爆（南美占画布高 228%、非洲 164%）——**这是改动前就有的老问题**（旧公式同样只算宽度），本次一并修掉。现取宽高两个约束里更严格的那个（`fitZoomForLngLatBox`），实测六洲在五种画布下**全部 0/6 溢出**（改前 2–3/6）。
      - 像素换算的分母必须是 **ECharts 实际使用的投影矩形宽**，不是真实投影宽度：两者实测相差 **1.2447 倍**（4.2843 vs 5.3325）。`zoom` 的语义是「相对这个矩形」，故必须用 `getBoundingRect()` 的值（`pixelsPerProjectedUnit`）。若误用真实宽度，所有按 zoom 换算的取景都会系统性偏大 1.24 倍。
      - **`pointToData` 在启用投影后是坏的（ECharts 5.6 缺陷）**：`lib/coord/geo/Geo.js` 的 `pointToData` 先 `projection.unproject(point)` 再 `this.pointToProjected(point)`，**顺序反了** —— 原始**像素**被直接喂给 `unproject`。实测往返 0/6 正确（纬度恒塌缩成 -1.395）；正确顺序（先逆 roam、再 unproject）6/6 精确。当前唯一调用点是 `cull.ts` 的 `viewportBox`，而世界图裁剪被 early-return，故**目前是潜伏缺陷**；已在 `cull.ts` 改为 `pointToProjected` + `projection.unproject` 组合，将来开世界图裁剪时不会踩。注意开世界图裁剪还需把「取画布四角」改成多采样（非线性映射下四角不对应数据极值）。
      - **已发现的 ECharts 采样缺陷（不产生裁剪，但代价是横向比例）**：ECharts 用 `geoCreator.js` 的 `sampleLine` 采样 `boundingCoords` 求投影包围盒时，「Left」边走的是 (xMin,yMax)→(xMax,yMin) 的**对角线**而非竖直线，非线性投影下会算得偏窄（实测 rect 宽 4.2843 vs 真实 5.3325）。遍历全部 73,370 个顶点核对：各种配置下最西/最东/最南/最北顶点均落在画布内、左右留白对称（900×460 下世界占宽 82.0%、留白 80.9/80.9px），**不产生裁剪或错位**；实际代价是世界图被画得比理想窄约 24%（改动前的等距圆柱基线同样受此影响，反而只占 63.6% 宽，故本次并未变差）。保留 `boundingCoords` 的理由是取景基准确定、可复核；若将来想满宽，可对世界图去掉它（安全——世界族单档、运行时几何不变），但**必须同时改用它那时的矩形宽**。
