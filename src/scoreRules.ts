...
import { continentFromScope, WORLD_NATION_SCOPE } from './province';

/**
 * 成绩提交资格规则（单一事实源，前端结算/提交用）。
 * 与 functions/_lib/validate.ts 的 validateScore 保持语义一致：
 * - endless 需有金币；
 * - 全国 self/click（scopeProvince=null）、「世界全国」（哨兵 `__world_nation__`）与
 *   各大洲榜（哨兵 `__continent_XX__`）允许未答完（已答全对即可，wrong 必须为 0）；
...
export function canSubmitScore(result: RoundResult): boolean {
  ...
  if (result.scopeProvince === null || result.scopeProvince === WORLD_NATION_SCOPE || continentFromScope(result.scopeProvince) !== null) {
