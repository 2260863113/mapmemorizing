 ...
 * - endless 需有金币；
 * - 全国 self/click（scopeProvince=null）与「世界全国」（哨兵 `__world_nation__`）
 *   允许未答完（已答全对即可，wrong 必须为 0）——世界榜与市级全国同语义；
...
export function canSubmitScore(result: RoundResult): boolean {
  ...
  if (result.scopeProvince === null || result.scopeProvince === WORLD_NATION_SCOPE) return result.correct > 0 && result.wrong === 0;
